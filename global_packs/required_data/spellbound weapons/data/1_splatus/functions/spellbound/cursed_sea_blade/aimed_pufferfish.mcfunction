...
execute at @s if score @s spellbound_count matches 20.. if predicate 1_splatus:chance/2_percent run kill @s
execute at @s if entity @e[sort=nearest,limit=1,distance=..3,type=!#1_splatus:spellbound/misc,type=!#1_splatus:spellbound/fish,predicate=!1_splatus:holding/weapon/cursed_sea_blade] if predicate 1_splatus:chance/5_percent run kill @s