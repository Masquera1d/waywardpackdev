...
execute if score @s spellbound_count matches 1.. run tp @s ^ ^ ^.35

execute if score @s spellbound_count matches 1..5 run particle flash ~ ~ ~ 0 0 0 0 2 force @a[distance=..64]
...
particle electric_spark ~ ~ ~ 1 1 1 .5 1 force @a[distance=..64]
execute if predicate 1_splatus:chance/50_percent run particle end_rod ~ ~ ~ .8 .8 .8 0 1 force @a[distance=..64]
scoreboard players add @e[distance=..3.5,type=!area_effect_cloud,type=!marker] spellbound_crucible_stunned 0
scoreboard players set @e[distance=..3.5,type=!area_effect_cloud,type=!marker,scores={spellbound_crucible_stunned=0},nbt=!{NoAI:1b}] spellbound_crucible_stunned 1
