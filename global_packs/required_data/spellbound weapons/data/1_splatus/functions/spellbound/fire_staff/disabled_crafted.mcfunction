recipe take @s 1_splatus:fire_staff
advancement revoke @s only 1_splatus:fire_staff
execute if entity @s[nbt={Inventory:[{id:"minecraft:knowledge_book",Count:1b,tag:{CustomModelData:47007}}]}] run give @s fishing_rod{CustomModelData:47009,FireStaff:1,display:{Name:'{"translate":"Fire Staff","color":"#ff371c","italic":false}',Lore:['{"translate":"Shoot fireballs and set flames","color":"#ff6652","italic":true}','{"translate":"","color":"gray","italic":false}','{"translate":"When In Main Hand:","color": "gray","italic":false}','{"translate":" 6 Attack Damage (Initial Strike)","color":"dark_green","italic":false}','{"translate":" 3 Second Default Charge Time","color":"dark_green","italic":false}']},StoreDamage:64f}
clear @s knowledge_book{CustomModelData:47007} 1
execute if entity @s[nbt={Inventory:[{id:"minecraft:knowledge_book",Count:1b,tag:{CustomModelData:47007}}]}] run function 1_splatus:spellbound/fire_staff/crafted

